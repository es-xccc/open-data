...
max_object_counts = {label: 0 for label in labels}
...
while True:
    ...
    if current_time - prev_time >= interval:
        ...
        with open('object_detection.csv', 'a', newline='') as file:
            writer = csv.writer(file)
            writer.writerow([datetime.now().strftime('%Y-%m-%d %H:%M:%S')] + list(max_object_counts.values()))
        max_object_counts = {label: 0 for label in labels}  # Reset max counts after writing to CSV
